...
transformed parameters {
    real lambda_squared = lambda^2;
}
model {
    target += - log(sigma); // improper 1/sigma prior
    lambda_squared ~ gamma(delta, r);
    target += n * ( log(lambda) - log(sigma) ) - ( lambda * norm1(beta) / sigma ); // laplace prior on beta
    y ~ normal(mu + standardized_x * beta, sigma);
}
generated quantities {
    array[n] real y_pred = normal_rng(mu + standardized_x * beta, sigma);
}